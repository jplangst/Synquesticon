    ...
    return (
      ...
      >
      {
        this.taskList.map((item, index) => {
          return <div key={index}><TaskItemComponent placeholder={false} task={item} removeCallback={this.onRemoveTask.bind(this)} onSelectedCallback={this.onSelectTask.bind(this)}/></div>
        })
      }
      </Reorder>
    );
